...
def create_mix(idx, triplet, snr_levels, out_dir, test=False, sr=16000, **kwargs):
    ...
    path_mix = os.path.join(
        out_dir, f"mix/{target_id}_{noise_id}_" + "%06d" % idx + "-mixed.wav"
    )
    path_target = os.path.join(
        out_dir, f"targets/{target_id}_{noise_id}_" + "%06d" % idx + "-target.wav"
    )
...
class MixtureGenerator:
    def __init__(
        ...
        if not os.path.exists(os.path.join(self.out_folder, "mix")):
            os.makedirs(os.path.join(self.out_folder, "mix"))
        if not os.path.exists(os.path.join(self.out_folder, "targets")):
            os.makedirs(os.path.join(self.out_folder, "targets"))
        if not os.path.exists(os.path.join(self.out_folder, "refs")):
            os.makedirs(os.path.join(self.out_folder, "refs"))
